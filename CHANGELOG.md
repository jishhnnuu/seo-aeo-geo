...
- The plugin-runtime setup step in every one of the five agent jobs hardcoded
  `TAG="v1.6.0"` for fetching `requirements.txt`, unrelated to the actual
  release each job otherwise pinned against elsewhere in the same file. Every
  occurrence now tracks the current release.
