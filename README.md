...
- **Can fix, not just report.** `/seo fix` applies the safe, deterministic findings directly — dry-run by default, git-aware, backed up, idempotent, re-verified after writing.
...
| `/seo fix <url|path> [--category ...] [--dry-run]` | Opt-in fixer for AUTO-class findings. Read-only by default (dry-run); writes only after explicit confirmation. The only command in this plugin that can modify files — see [NOTICE.md](./NOTICE.md). |
| `/seo aso <app store url>` | App Store / Google Play listing audit — a separate ranking surface from web/AI search, covered here since it's part of "rank at the top wherever people search." |
...
Optional MCP servers add live data to the audit pipeline. This plugin ships extensions for 8 servers; the plugin core works without any of them.
...
The base audit engine (inherited from AgriciDaniel/claude-seo) includes contributions from that project's AI Marketing Hub Pro Hub Challenge:
...
MIT License — see [LICENSE](LICENSE). This is a derivative work merging four MIT-licensed upstream projects; see [NOTICE.md](./NOTICE.md) for the full provenance breakdown and each upstream project's preserved license text (`LICENSE-claude-seo`, `LICENSE-claude-seo-ai`, `LICENSE-claude-skill-seo-geo-optimizer`, `LICENSE-marketingskills`).
...
This repository merges and adapts code from four separately-maintained,
MIT-licensed open-source projects. All original engineering credit belongs
to their respective authors:

- **[AgriciDaniel/claude-seo](https://github.com/AgriciDaniel/claude-seo)** by Agrici Daniel — base audit engine and 24 core skills.
- **[Hainrixz/claude-seo-ai](https://github.com/Hainrixz/claude-seo-ai)** by Enrique Rocha — the opt-in fixer and confidence-tiered finding schema.
- **[199-biotechnologies/claude-skill-seo-geo-optimizer](https://github.com/199-biotechnologies/claude-skill-seo-geo-optimizer)** by Boris Djordjevic — the actively-corrected GEO/AEO statistics and platform-citation references.
- **[coreyhaines31/marketingskills](https://github.com/coreyhaines31/marketingskills)** by Corey Haines — the content-strategy, competitor-profiling, and programmatic-SEO business-context layers.

Full breakdown of what came from where, including original additions made
after the merge: [NOTICE.md](./NOTICE.md).

### A fifth, non-merge addition worth knowing about

After the initial four-way merge, a deep comparison against the wider
Claude-Code-SEO tool ecosystem (~10 other tools) turned up two real gaps:
a more rigorous, peer-reviewed GEO scoring methodology, and a genuinely
dangerous blind spot (Cloudflare silently blocking AI crawlers at the edge
since July 2025, undetectable from `robots.txt` alone). Both are now in
this plugin — see `skills/seo-geo/references/peer-reviewed-geo-research.md`,
`scripts/geo_content_score.py`, and NOTICE.md section 5 for the full story.
