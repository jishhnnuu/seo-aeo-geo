<!-- Sourced from 199-biotechnologies/claude-skill-seo-geo-optimizer (MIT), adapted for this repo. See /NOTICE.md for full attribution. -->

> **Staleness caveat (added for this repo):** this guide is dated 2025 and
> predates two relevant changes covered elsewhere in this plugin. First,
> Google retired FAQ rich results in Search on 2026-05-07 (see
> `skills/seo-geo/references/tracked-statistics-2026.md` and
> `skills/seo-schema/references/deprecated-types-2024-2026.md`) — treat
> any "FAQ schema = highest voice eligibility" claim below as historical,
> not current guidance for Google surfaces; it may still hold for other
> voice assistants that don't share Google's rich-result pipeline, but
> verify per-assistant rather than assuming. Second, any specific
> percentage-lift claim below (e.g. "+35%", "+40%") should be treated as
> `speculative` per this repo's confidence-tier discipline
> (`schema/finding.schema.json`) unless independently re-verified — several
> similarly-worded claims from the same 2025 vintage were retracted in the
> tracked-statistics file above. The structural guidance (Speakable schema,
> 29-word answer format, question-based content) is still sound; the
> specific numbers around it need re-checking before being cited in a
> report.
