...
### 9b. Fix ordering — a default when the dependency graph doesn't dictate one

Section 6's "dependency graph, not a list" discipline already covers
cases where one fix genuinely blocks another. For everything else,
default to this order — added for this repo after reviewing how other
audit tools in this space sequence their own priority matrices, since
"prioritized by impact" without a concrete default still leaves room for
arbitrary ordering:

1. **Crawl/index blocking issues first** — robots.txt errors, accidental
   `noindex`, 5xx server errors. If Google (or any engine) can't crawl or
   index the page at all, nothing downstream matters; fixing schema on an
   unindexed page is wasted effort.
2. **AI crawler access, if AI visibility is a stated goal** — including
   the edge-WAF check in `skills/seo-geo/SKILL.md` ("Critical: robots.txt
   is not sufficient evidence of AI crawler access"). Blocked discovery
   means no citation, structurally identical to step 1 but AI-surface
   specific, so it belongs immediately after classic crawlability, not
   buried further down the list.
3. **Core Web Vitals failures on key templates** (LCP, INP, CLS) — a
   page that's indexed but fails CWV thresholds is still being actively
   penalized in ranking and in AI-answer eligibility.
4. **Duplicate content / canonical conflicts** that split link equity or
   confuse which version an engine should treat as authoritative.
5. **Site architecture** (click depth, internal linking, orphan pages) —
   real, but only worth prioritizing once the above are clean; architecture
   work on a site with active indexing/crawlability problems is premature.

This is a default, not a rule — a genuine dependency (e.g. a migration
mid-flight, per `skills/seo-drift/SKILL.md`'s migration-validation
checklist) can override it, but override deliberately, not by omission.
