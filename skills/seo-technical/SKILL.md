...
- Crawl budget: for large sites (>10k pages), efficiency matters — if the
  user can supply server access logs (Apache/Nginx Combined Log Format
  export from their host or CDN), run `scripts/analyze_crawl_logs.py
  --file <log> --json` for ground-truth crawl-budget evidence: real bot
  vs. human traffic split, per-bot status-code breakdown (a bot burning
  requests on 3xx/4xx/5xx instead of indexable 200s is wasted budget,
  concretely), and — notably — whether AI retrieval bots (OAI-SearchBot,
  Claude-SearchBot, PerplexityBot) have actually visited at all, which a
  permissive robots.txt or a clean live-access check
  (`scripts/check_ai_crawler_access.py`) cannot show; permission to crawl
  and actually being crawled are different facts, and this is the only
  check in this plugin that can tell them apart.
