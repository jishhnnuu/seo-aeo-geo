...
## Site migrations are a drift-comparison use case, not a separate skill

If the user is planning or has just completed a site migration (domain
change, platform move, URL restructure), that's this skill with a
specific checklist, not a different capability. Run `/seo drift baseline
<url>` against representative pages on the **old** structure before
cutover, then `/seo drift compare <url>` against the **new** URLs after,
and check specifically for what migrations most commonly break — none of
which the generic drift diff surfaces by default, so call them out
explicitly in the comparison report:

- **Redirect chains**: old URL → new URL should be a single 301, not a
  chain through two or three intermediate redirects (each hop costs
  crawl budget and can drop link equity).
- **Canonical consistency**: the new page's canonical should point to
  itself, not still reference the old URL or domain.
- **Title/meta preservation**: verify title tags and meta descriptions
  carried over intentionally rather than reverting to a CMS default
  during the platform move — this is a common, easy-to-miss migration
  casualty.
- **HTTP status codes**: confirm old URLs 301 rather than 404ing outright,
  and that the new URLs return clean 200s rather than soft-404s.
