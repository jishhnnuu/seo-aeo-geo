...
name: seo-aso
description: >
  App Store Optimization (ASO) audit for App Store and Google Play listings
  — a separate ranking surface from web/AI search but part of the same
  "rank at the top wherever people search" goal this plugin covers. Use
  when the user wants to audit or optimize an App Store or Google Play
  listing, or mentions "ASO audit", "app store optimization", "optimize
  my app listing", "improve app visibility", "app store ranking", "why
  aren't people downloading my app", "keyword optimization for app", or
  "compare my app to competitors". Also trigger when the user shares an
  App Store or Google Play URL and wants it improved.
user-invocable: true
...
Within this plugin: `/seo schema` and `/seo geo` cover the web-search and
AI-citation surfaces this app's own marketing site should also be optimized
for — an app with a strong ASO score but a weak or unindexed marketing
site is still losing organic discovery on the web side. `/seo content-brief`
can turn app-store keyword findings from this skill into a content brief
for a companion landing page.

Landing-page conversion optimization, paid app-install ad creative, and
install-attribution analytics are out of scope for this plugin — those
belong in a separate marketing-automation toolkit if you build one
(see this plugin's README for that recommendation).
