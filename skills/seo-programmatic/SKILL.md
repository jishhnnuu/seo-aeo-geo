...
## Data defensibility and business framing (before templating)

*Merged from coreyhaines31/marketingskills' `programmatic-seo` skill (MIT). See /NOTICE.md.*

Data-source assessment (above) checks whether the data is *usable*.
Before committing to a template/URL-pattern strategy, also weigh whether
the data is *defensible* — this determines how much competitive moat the
resulting pages actually have:

**Data defensibility hierarchy (strongest to weakest):**
1. **Proprietary** — data the business created itself (usage stats,
   original research, internal benchmarks)
2. **Product-derived** — data generated by the product/users in aggregate
3. **User-generated** — community contributions specific to this site
4. **Licensed** — exclusive access to a third-party dataset
5. **Public** — anyone can build the same pages from the same source;
   weakest position, easiest for a competitor to replicate at scale

Favor proprietary and product-derived sources when a choice exists; flag
in the plan when a proposed template set relies entirely on public data,
since that's the scenario most likely to be outranked by whoever builds a
similar template with better content depth or existing domain authority.

**Frame the plan around business context, not just data mechanics** —
before finalizing template count and URL patterns, confirm: what's the
conversion goal for these pages, what's the realistic page count given
search volume distribution across the pattern, and who currently ranks
for representative examples of the pattern (can this site realistically
compete, or is the segment already saturated by an incumbent with a
similar programmatic set).
