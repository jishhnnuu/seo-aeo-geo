...
## Business-context layer (before briefing individual keywords)

*Merged from coreyhaines31/marketingskills' `content-strategy` skill (MIT). See /NOTICE.md.*

The sections above turn one keyword cluster into one brief. Before doing
that repeatedly, spend one pass establishing the strategy those briefs
should serve — otherwise you'll produce technically-correct briefs that
don't add up to a coherent content program:

- **Searchable vs. shareable, in that order.** Prioritize content that
  captures existing demand (search traffic) before content designed to
  create demand (shareable/thought-leadership). Search is the foundation;
  shareable content works best once there's a searchable base to link it
  to internally.
- **Source topics from where customers actually talk**, not just keyword
  tools: recurring questions from sales calls, objections that come up
  before close, and patterns in support tickets. These surface intent and
  language keyword-volume data alone won't show, and often reveal
  higher-converting long-tail topics than pure SERP research.
