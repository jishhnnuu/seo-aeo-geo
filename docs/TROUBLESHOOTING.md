...
```bash
curl -fsSL https://raw.githubusercontent.com/jishhnnuu/seo-aeo-geo/main/install.sh | bash
```
...
```bash
curl -fsSL https://raw.githubusercontent.com/jishhnnuu/seo-aeo-geo/main/requirements.txt \
...
For plugin installs, check `/plugin list` and reinstall `claude-seo-unified@jishnu-seo-aeo-geo`; subagents load from the plugin, not `~/.claude/agents/`.
